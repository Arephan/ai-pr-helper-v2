...
exports.getComplexityEmoji = getComplexityEmoji;
const types_js_1 = require("../types.js");
/**
 * Analyze code complexity and return metrics + suggestions
 */
function analyzeComplexity(code, thresholds = types_js_1.DEFAULT_CONFIG.complexityThresholds) {
 ...
 */
function calculateMaxParameters(code) {
    let maxParams = 0;
 ...
 */
function calculateLineCount(code) {
 ...
 */
function calculateDependencyCount(code) {
 ...
 */
function calculateScore(metrics, thresholds) {
    // Normalize each metric to 0-2 range based on threshold
    const normalize = (value, threshold) => {
 ...
 */
function generateSuggestions(metrics, flags, code) {
    const suggestions = [];
 ...
 */
function getComplexityEmoji(score) {
    if (score <= 3)
        return '🟢';
    if (score <= 5)
        return '🟡';
