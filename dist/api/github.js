...
function buildReviewSummary(result, inlineCount) {
    const parts = [];
    parts.push('## 🔍 AI Review Helper Analysis');
    parts.push('');
    // Quick stats
    ...
    if (!hasIssues) {
        parts.push('✨ **Looking good!** No major concerns found in this PR.');
        parts.push('');
        ...
        if (inlineCount > 0) {
            parts.push('');
            parts.push(`📝 See ${inlineCount} inline comment${inlineCount === 1 ? '' : 's'} for specific suggestions.`);
        }
    }
    parts.push('');
    parts.push('---');
    parts.push('<sub>🤖 Generated by AI Review Helper | [Configuration](https://github.com/your-org/ai-review-helper)</sub>');
    return parts.join('\n');
}
/**
