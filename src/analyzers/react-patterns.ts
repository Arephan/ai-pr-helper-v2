 ...
 */
export function analyzeReactPatterns(code: string, filename: string): ReactAnalysis {
...
function detectComponentType(code: string): 'functional' | 'class' | 'unknown' {
  if (/extends\s+(React\.)?Component/.test(code)) return 'class';
  if (/function\s+\w+.*?{|const\s+\w+\s*=\s*\(/.test(code)) return 'functional';
  return 'unknown';
}
...
function detectNestedTryCatch(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
...
function detectUseEffectIssues(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
...
function detectStateIssues(code: string, lines: string[], stateCount: number): ReactPattern[] {
  const patterns: ReactPattern[] = [];
...
function detectMemoIssues(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
...
function detectCallbackIssues(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
...
function detectOverDefensiveCode(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
...
function detectVerboseComments(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
  ...
  for (const pattern of syntaxComments) {
    if (pattern.test(code)) verboseCount++;
  }
...
function detectAIPatterns(code: string, lines: string[]): ReactPattern[] {
  const patterns: ReactPattern[] = [];
 ...
 */
export function generateComponentDiagram(code: string): string {
