...
import { ReviewResult, HunkAnalysis, FileAnalysis } from '../types.js';
 ...
 */
export function formatGithubActionsOutput(result: ReviewResult): string {
  const lines: string[] = [];
  ...
  const filesWithIssues = result.files.filter(f => 
    f.hunks.some(h => 
      (h.patterns?.patternsFound.length ?? 0) > 0 ||
 ...
 */
export function formatMinimalSummary(result: ReviewResult): object {
  return {
    files: result.files.length,
    hunks: result.totalHunks,
    aiLikelihood: result.aiCodeLikelihood,
    issues: {
      patterns: result.files.reduce((acc, f) => 
        acc + f.hunks.reduce((a, h) => 
          a + (h.patterns?.patternsFound.length ?? 0), 0
        ), 0
      ),
      highComplexity: result.files.reduce((acc, f) =>
        acc + f.hunks.filter(h => (h.complexity?.score ?? 0) > 5).length, 0
      )
    },
...
export function formatCiResult(
  ...
  const {
    maxPatterns = 10,
    maxComplexity = 8,
    maxAiLikelihood = 'high'
  } = thresholds;
  
  const reasons: string[] = [];
  
  // Count patterns
  const patternCount = result.files.reduce((acc, f) => 
    acc + f.hunks.reduce((a, h) => 
      a + (h.patterns?.patternsFound.length ?? 0), 0
    ), 0
  );
  ...
  for (const file of result.files) {
    for (const hunk of file.hunks) {
      if (hunk.complexity && hunk.complexity.score > maxComplexity) {
        reasons.push(
          `High complexity in ${file.filename}:${hunk.hunk.startLine} ` +
  ...
  if (likelihoodLevels[result.aiCodeLikelihood] > likelihoodLevels[maxAiLikelihood]) {
    reasons.push(
      `AI likelihood too high: ${result.aiCodeLikelihood} (max: ${maxAiLikelihood})`
    );
  }
